...
R.layOutStage = () => {
    ...
    const SpreadGap = S['reader-view-mode'] == 'paged' || B.Package.Metadata['rendition:flow'] == 'scrolled-continuous' || B.Package.Metadata['scroll-direction'] ? 0 : Math.max(Math.ceil(R.Stage[C.L_SIZE_L] / 8), 40);
...
R.focusOn = (_, Opt) => new Promise((resolve, reject) => {
    ...
    R.Moving = true;
    let FocusPoint;
    const Page = _.Page, Item = Page.Item;
    if(Item.Reflowable) {
        if(!R.Columned && _.Element) {
            ...
            if(_.Side == 'after') FocusPoint += (Page['offset' + C.L_SIZE_L] - R.Stage[C.L_SIZE_L]) * C.L_AXIS_D;
        ...
        if(R.Stage[C.L_SIZE_L] >= Page.Spread['offset' + C.L_SIZE_L]) {
            ...
            else if(_.Side == 'after') FocusPoint += (Page['offset' + C.L_SIZE_L] - R.Stage[C.L_SIZE_L]) * C.L_AXIS_D;
        }
    }
