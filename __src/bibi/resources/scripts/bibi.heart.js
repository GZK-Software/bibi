...
R.focusOn = (_, Opt) => new Promise((resolve, reject) => {
    ...
    if(S['book-rendition-layout'] == 'reflowable') {
        if(!R.Columned && _.Element) {
            FocusPoint = O.getElementCoord(_.Item)[C.L_AXIS_L] + S['item-padding-' + C.L_OOBL_l] + _.Element.getBoundingClientRect()[C.L_BASE_b];
        } else {
            FocusPoint = O.getElementCoord(_.Page)[C.L_AXIS_L];
            if(_.Side == 'after') FocusPoint += (_.Page['offset' + C.L_SIZE_L] - R.Stage[C.L_SIZE_L]) * C.L_AXIS_D;
            if(S.SLD == 'rtl') FocusPoint += _.Page.offsetWidth;
        }
        if(S.SLD == 'rtl') FocusPoint -= R.Stage.Width;
    } else {
        if(R.Stage[C.L_SIZE_L] >= _.Page.Spread['offset' + C.L_SIZE_L]) {
            FocusPoint = O.getElementCoord(_.Page.Spread)[C.L_AXIS_L];
            FocusPoint -= Math.floor((R.Stage[C.L_SIZE_L] - _.Page.Spread['offset' + C.L_SIZE_L]) / 2);
        } else {
            FocusPoint = O.getElementCoord(_.Page)[C.L_AXIS_L];
            if(R.Stage[C.L_SIZE_L] > _.Page['offset' + C.L_SIZE_L]) FocusPoint -= Math.floor((R.Stage[C.L_SIZE_L] - _.Page['offset' + C.L_SIZE_L]) / 2);
            else if(_.Side == 'after') FocusPoint += (_.Page['offset' + C.L_SIZE_L] - R.Stage[C.L_SIZE_L]) * C.L_AXIS_D;
        }
    }
...
R.dest = (_, Opt) => { if(_ === undefined || _ === null) return null;
    ...
    const DD = { /* // Distilled Destination
    ...
    */ };
    if(Opt && Opt.Distilled) {
        ...
        switch(typeof _) {
            case 'object': _ = _.nodeType == 1 ? { Element: _ } : Object.assign({}, _); break;
    ...
    const Page = !Object.keys(DD).length ? _.Page :
        ...
        DD.Item     ? R.getPageIn(DD.Item,   DD.ProgressInItem)   :
        DD.Spread   ? R.getPageIn(DD.Spread, DD.ProgressInSpread) :
                      R.getPageIn(   R,      DD.Progress)         ;
    if(Page && Page.IsPage) {
        DD.Page = Page;
        return DD;
    }
    ...
    R.getPageIn = (In, Progress) => In.Pages[ 0 < Progress ? sML.limitMax(Math.floor(In.Pages.length * Progress), In.Pages.length - 1) : 0 ];
...
R.getElement = (_, Opt) => {
    ...
    R.getElementInPage = (Page, Opt) => {
        ...
        while(_Ele && _Ele != Item.Body) if(REs.Suitable_Div.test(_Ele.tagName)) return _Ele;
        return Ele;
    };
...
I.TextSetter = { create: () => {
    ...
    const TextSetter = I.TextSetter = {
        change: (Par, Actions) => new Promise(resolve => {
            ...
            if(!TextSettingProperties.filter(Pros => { const ProC = Pros[0], ProH = Pros[1];
                ...
                E.dispatch('bibi:changes-' + ProH); // --> E.dispatch('bibi:changes-font-size'), E.dispatch('bibi:changes-line-height')
                return true;
            ...
            setTimeout(() => R.layOutBook({
                ...
                TextSettingProperties.forEach(Pros => { const ProC = Pros[0], ProH = Pros[1];
                    if(Par[ProC]) E.dispatch('bibi:changed-' + ProH, Par[ProC]); // --> E.dispatch('bibi:changed-font-size', Par.FontSize), E.dispatch('bibi:changed-line-height', Par.LineHeight)
                });
    ...
    TextSettingProperties.forEach(Pros => { const ProC = Pros[0], ProH = Pros[1];
        const ScalePerStepSetting = S[ProH + '-scale-per-step']; // --> S['fontsize-scale-per-step'], S['lineheight-scale-per-step']
        TextSetter[ProC] = {
    ...
    E.bind('bibi:postprocessed-item', Item => { if(Item.PrePaginated) return false;
        ...
        const ItemTextSetter = Item.TextSetter = {
            ...
            LineHeight: {
                ...
                changeScale: (Scale) => { if(!Number.isFinite(Scale) || Scale <= 0) return;
                    ItemTextSetter.LineHeight.CustomizableElements.forEach(Ele => Ele.style.lineHeight = Ele.BibiTextSetter.LineHeight.Base * Scale);
                }
            }
        };
        TextSettingProperties.forEach(Pros => { const ProC = Pros[0], ProH = Pros[1];
        ...
        sML.forEach(Item.HTML.querySelectorAll('body, body *:not(script):not(style):not(img):not(svg):not(picture):not(iframe):not(audio):not(video):not(source)'))(Ele => {
            ...
            ItemTextSetter.LineHeight.CustomizableElements.push(Ele);
        });
        Item.HTML.style.fontSize = ItemTextSetter.FontSize.Base + 'px';
        ItemTextSetter.LineHeight.CustomizableElements.forEach(Ele => Ele.style.lineHeight = Ele.BibiTextSetter.LineHeight.Base);
        ItemInitializers.forEach(initializer => initializer(Item));
    });
    if(S['use-fontsize-changer-ui'] || S['use-lineheight-changer-ui']) {
        ...
        if(S['use-lineheight-changer-ui']) {
            ...
            (TextSetter.LineHeight.Buttons = Subpanel.addSection({
                Labels: { default: { default: `Line Spacing <small>(Line Height / Leading)</small>`, ja: `行間<small>（行高）</small>` } }
            }).addButtonGroup({
                Type: 'Steps', Of: 'LineHeight',
                ...
                if(Button.Step == TextSetter.LineHeight.Step) I.setUIState(Button, 'active');
            });
        }
    }
...
I.createButtonGroup = (Par = {}) => {
    ...
    if(Par.Type == 'Steps') {
        const BaseClassName = 'bibi-buttongroup-steps', UniqueClassName = BaseClassName + '-' + Date.now();
        ClassNames.push(BaseClassName);
        ClassNames.push(UniqueClassName);
        if(typeof Par.MinLabels == 'object') sML.CSS.appendRule('.' + UniqueClassName + ':before', `content: "` + I.distillLabels(Par.MinLabels)['default'][O.Language] + `" !important;`);
        if(typeof Par.MaxLabels == 'object') sML.CSS.appendRule('.' + UniqueClassName + ':after',  `content: "` + I.distillLabels(Par.MaxLabels)['default'][O.Language] + `" !important;`);
        ButtonsToBeAdded.forEach(Button => Button.Type = 'radio');
    } else if(Par.Type == 'Tiled' || Par.Tiled) {
        ClassNames.push('bibi-buttongroup-tiled');
    }
    if(Par.Sticky) ClassNames.push('sticky');
    Par.className = ClassNames.concat(typeof Par.className == 'string' && Par.className ? Par.className : []).join(' ');
